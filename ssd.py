...
class SSD(nn.Module):
    ...
    def forward(self, x):
        ...
        loc = list()
        conf = list()
        # input shape: SSD300 - (batch_size, 3, 300, 300), SSD512 - (batch_size, 3, 512, 512)
        # len(self.vgg)=35
...
extras = {
    '300': [256, 'S', 512, 128, 'S', 256, 128, 256, 128, 256, 128],
...
def build_ssd(phase, size=300, num_classes=21):
    ...
    if size not in [300, 512]:
        print("ERROR: You specified size " + repr(size) + ". However, " +
              "currently only SSD300 (size=300) and SSD512 (size=512) is supported!")
        return
